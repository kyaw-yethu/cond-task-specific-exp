...
**Visual**, in `cond_eval/dit_eval.py`: mean PSNR, SSIM and LPIPS over the predicted raw
...
`cond_eval/traj_metrics.py` and verified bit-exact against their code: ADE and FDE in
...
**The extractor is a supervised ego-motion probe**, `cond_eval/pose_probe.py`, not
...
`cond_eval/` is the library, `scripts/` the entry points and node runners, `tests/` the

...
Code: `cond_eval/pose_probe.py` (model), `cond_eval/traj_metrics.py` (the geometry),
`scripts/train_probe.py` (training and the floors).
...
Reported by `scripts/train_probe.py` before any comparison is read.
...
| `out/logs/probe_vaeaug.log`, `out/probe_clean.log` | full training logs, parsed by `scripts/plot_probe_curve.py` |

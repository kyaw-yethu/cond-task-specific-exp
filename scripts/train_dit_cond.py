...
Usage:
    python scripts/train_dit_cond.py --jdm third_party/Task_specific_JDM --dataset waymo \\
...
from cond_eval.cached import CachedCondDataset, read_cache_meta
from cond_eval.cond_dit import ConditionedDiT, CrossAttnConditioner, latent_cutoffs
...
def main():
    ...
    parser.add_argument("--context-width", type=int, default=6,
                        help="raw frames the encoder sees: the k=2 window, uniform "
                             "across the batch (see cond_eval.r_sources)")
    ...
    parser.add_argument("--cache-dir", default=None,
                        help="a scripts/build_cache.py directory. With it, k is FIXED at "
                             "the cache's value, the VAE and the r encoder are never run "
                             "during training, and the 17.7 GB of video is never read. "
                             "About 5.9x faster; see cond_eval.cached for what fixing k "
                             "costs")

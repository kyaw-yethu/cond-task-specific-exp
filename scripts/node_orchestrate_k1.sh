...
bash scripts/node_k1_chain.sh 0 G1:vjepa2:vjepa2_G_vith256:2      > out/logs/k1_chain0.log 2>&1 &
bash scripts/node_k1_chain.sh 1 D1:drivejepa:drivejepa_D_vitl256:2 > out/logs/k1_chain1.log 2>&1 &
bash scripts/node_k1_chain.sh 2 T1:drivejepa:drivejepa_T_vitl256:2 > out/logs/k1_chain2.log 2>&1 &
bash scripts/node_k1_chain.sh 3 V1:traj_vae:waypoint_vae_lr5x:1    > out/logs/k1_chain3.log 2>&1 &
bash scripts/node_k1_chain.sh 3 N1:none:-:0                        > out/logs/k1_chain4.log 2>&1 &
wait
log "all chains finished"
cat out/logs/k1_chain*.log

log "building the qualitative comparison on GPU 0"
CUDA_VISIBLE_DEVICES=0 python3 -u scripts/make_comparison.py \
...
python3 -u scripts/make_results.py --out-root "$PROJ/out" --proj "$PROJ" \
...
bash scripts/node_check_figs.sh || log "WARNING: some doc figures are missing"
log "ALL DONE"

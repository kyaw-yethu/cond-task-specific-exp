...
The geometry itself lives in ``conditioning.lib.traj_metrics`` rather than here, so it
...
metric becomes partly a blur detector. ``conditioning/scripts/train_probe.py`` measures that
...
def build_pose_fn(device, ckpt_path=None, start_from_truth=True):
    """Factory for ``conditioning/scripts/eval_dit.py --pose-module``.

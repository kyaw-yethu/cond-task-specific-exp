...
for SPEC in "$@"; do
  ...
  if [ -f "$CACHE_ROOT/$TAG/cache_meta.json" ]; then
    ...
    bash conditioning/scripts/node_build_cache.sh "$TAG" "$COND" $RUNARG --fixed-k 1 $WARG \
  ...
  if [ "$(evaluated "$TAG")" = "1" ]; then
    ...
    log "$TAG evaluating"
    bash conditioning/scripts/node_eval_cond.sh "$TAG" > "out/logs/k1_eval_$TAG.log" 2>&1 \
      && log "$TAG eval done" \

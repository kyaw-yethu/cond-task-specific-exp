...
metrics.json files that `conditioning/scripts/eval_cond.py` writes, so one script serves every
...
Usage:
    python conditioning/scripts/make_results.py --out-root <proj>/out --proj <proj>
    python conditioning/scripts/make_results.py --out-root <proj>/out --proj <proj> --suffix 1 \\
...
def main():
    ...
    A("video-prediction literature uses and which `conditioning/lib/dit_eval.py` documents.\n")

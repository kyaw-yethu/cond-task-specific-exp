#!/usr/bin/env python
"""Evaluate a trained MiniDiT checkpoint with ``conditioning.lib``'s metrics.

Mirrors ``Task_specific_JDM/scripts/eval_dit.py`` and swaps in
``conditioning.lib.dit_eval.run_dit_eval``, which adds the trajectory axis on top of
...
Usage:
    python conditioning/scripts/eval_dit.py --jdm third_party/Task_specific_JDM \\
...
from conditioning.lib.dit_eval import run_dit_eval
...
def main():
    ...
    def sample_fn(videos, shape_z, condition_latent, num_cond, target_frames):
        ...
        # closure's sake -- see conditioning/lib/dit_eval.py.
